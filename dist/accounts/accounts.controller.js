...
router.get('/', (0, authorize_1.default)(role_1.Role.Admin), getAll);
router.get('/:id', (0, authorize_1.default)(), getById);
router.post('/', (0, authorize_1.default)(role_1.Role.Admin), createSchema, create);
...
function revokeToken(req, res, next) {
    ...
    if (!req.user.ownsToken(token) && req.user.role !== role_1.Role.Admin) {
...
function getById(req, res, next) {
    if (Number(req.params.id) !== req.user.id && req.user.role !== role_1.Role.Admin) {
        return res.status(401).json({ message: 'Unauthorized' });
    }
